...
describe('shared/adapters/registry (D-24 / D-26)', () => {
  it('registry contains mock, openclaw, discord, slack, telegram, and feishu entries', () => {
    ...
    expect(adapterRegistry[4]?.id).toBe('telegram');
    expect(adapterRegistry[5]?.id).toBe('feishu');
  });
