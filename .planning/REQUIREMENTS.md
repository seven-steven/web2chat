...
- [x] **FND-02**: Service worker 仅在顶层同步注册 `chrome.runtime.onMessage` 等事件监听器，不出现 top-level await（防御 SW 生命周期陷阱）
- [x] **FND-03**: popup ↔ service worker ↔ content script 之间使用类型化消息协议（`zod` 校验）封装，跨上下文 RPC 输入/输出有明确 TypeScript 类型
