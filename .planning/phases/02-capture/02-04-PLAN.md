...
must_haves:
  ...
  key_links:
    - from: "tests/unit/extractor/description-fallback.spec.ts"
      to: "entrypoints/extractor.content.ts (Wave 2 并行)"
    ...
    - from: "tests/unit/messaging/capture.spec.ts"
      ...
      via: "mirror 函数 capturePipelineCore 与 Wave 3 实现保持一致"
...
Purpose: TDD 驱动——测试文件先于或与实现并行编写（Wave 2 并行），为 Wave 3 的 SW pipeline 实现提供红灯参考，同时为 CAP-02/03/04 提供自动化验收。
Output: 4 个单元测试文件；pnpm test 通过（extractor 三个测试需要 Wave 2 的 extractor.content.ts 已存在且导出 getDescription）。
</objective>
...
From entrypoints/extractor.content.ts (Wave 2 并行创建):
```typescript
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - entrypoints/extractor.content.ts (确认 getDescription 导出路径)
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - shared/messaging/result.ts (ErrorCode 联合，确认 Wave 2 扩展已完成)
    - shared/messaging/protocol.ts (ArticleSnapshot 类型，确认 Wave 2 已追加)
 ...
 *   - background/capture-pipeline.ts (Wave 3) exports runCapturePipeline, but that
