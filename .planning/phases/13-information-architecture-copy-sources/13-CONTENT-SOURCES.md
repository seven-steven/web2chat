...
| CLM-PLATFORM-01 | Current supported platforms | Shipped platform set is OpenClaw, Discord, Slack, Telegram | "当前支持：OpenClaw / Discord / Slack / Telegram"; "shipped platforms" | "支持所有聊天平台"; claiming Feishu/Lark as supported; implying unshipped platforms | `.planning/PROJECT.md` Current shipped platform set; `.planning/ROADMAP.md` v1.2 scope | `rg "Current shipped platform set" .planning/PROJECT.md` | Update when PROJECT.md shipped platform set or ROADMAP v1.2 scope changes |
| CLM-PRIVACY-01 | Privacy / permissions trust | web2chat uses user-triggered capture, local storage, direct browser delivery, no remote server, no telemetry, no third-party analytics | "user-triggered capture"; "local storage"; "direct browser delivery"; "no remote server"; "no telemetry"; "no third-party analytics"; "用户主动点击"; "本地存储"; "直接浏览器投递"; "无遥测"; "无第三方分析" | "cloud sync"; "remote server"; "our servers"; "usage analytics"; "user tracking"; "云端存储"; "用户行为分析"; "server-side processing" | `PRIVACY.md` Data We Collect, How Data Is Stored, How Data Is Transmitted, Data We Do NOT Collect | `rg "only when you click\|No data is stored on any remote server\|do not operate or communicate with any remote server\|third-party analytics" PRIVACY.md` | Update when PRIVACY.md privacy model changes |
| CLM-PERM-01 | Privacy / permissions trust | Production permissions: `activeTab`, `alarms`, `scripting`, `storage`, `webNavigation`; production host permissions for known public platforms; optional origin grant for user-deployed instances | "activeTab"; "alarms"; "scripting"; "storage"; "webNavigation"; "optional origin grant"; listing production host permissions `https://app.slack.com/*`, `https://slack.com/*`, `https://discord.com/*`, `https://web.telegram.org/*` | production `tabs` permission; static production `<all_urls>` host permission | `wxt.config.ts` production manifest permissions and host_permissions arrays; `pnpm verify:manifest` | `pnpm verify:manifest` validates production manifest shape | Update when `wxt.config.ts` production manifest changes; always run `pnpm verify:manifest` after |
| CLM-LIMIT-01 | Known limits | Telegram is shipped but has live UAT pending / known risk status | "live UAT pending / known risk"; "Telegram shipped with live UAT pending"; "live session UAT 待补" | "fully verified Telegram"; "Telegram 完全验证"; using Telegram as primary product proof | `.planning/PROJECT.md` Known closeout gaps; `.planning/STATE.md` Deferred Items | `rg "Telegram.*缺真实" .planning/PROJECT.md` | Update when Telegram live UAT is completed or status changes |
| CLM-LIMIT-02 | Known limits | Feishu/Lark was evaluated and dropped from shipped scope; Phase 11/12 Nyquist partial is known risk only | "evaluated and dropped from shipped scope due to unreliable shared URL targeting"; "known risk only"; "评估后未纳入发布范围"; "已知风险" | "支持飞书/Lark"; "Feishu/Lark supported"; Feishu/Lark in supported platform badges; turning Nyquist partial into marketing feature | `.planning/PROJECT.md` Out of Scope / Deferred; `.planning/STATE.md` feishu-lark-final-scope-sync; `.planning/STATE.md` phase-11-12-nyquist-closeout | `rg "Feishu.*Lark.*dropped\|feishu-lark" .planning/STATE.md` | Update when Feishu/Lark is re-evaluated or Nyquist closeout status changes |
