...
- [x] **Phase 6: i18n 加固 + 打磨** ✓ Complete (2026-05-07) - 6/6 plans, 225 单元测试 + 100% locale 双向覆盖
- [x] **Phase 7: 分发上架** ✓ Complete (2026-05-07) - 4/4 plans, 构建验证脚本 + 双语隐私政策 + 双语 README + CWS Store Listing，`<owner>` 替换为 `seven-steven`，UAT 3/3 通过
...
**Wave 4** *(blocked on Wave 3 completion)*
- [x] 02-05-PLAN.md — SW capture-pipeline + background.ts 注册 + locale 键补齐（CAP-01 / CAP-02 / CAP-03 / CAP-04）✓ Plan 02-05 (2026-04-30)

**Wave 5** *(blocked on Wave 4 completion)*
- [x] 02-06-PLAN.md — Popup App.tsx 4-state capture UI（CAP-05）✓ Plan 02-06 (2026-04-30)

**Wave 6** *(blocked on Wave 5 completion)*
- [x] 02-07-PLAN.md — E2E fixture + capture.spec.ts + playwright webServer（CAP-01 / CAP-05）✓ Plan 02-07 (2026-04-30 — E2E spec 落地；首次跑待 human gate)
...
**Plans**：6 plans（4 waves — 原始 4 plans + 2 gap closure plans）

Plans:

**Wave 1** *(并行)*
- [x] 04-01-PLAN.md — Foundation: ErrorCode 扩展 + dom-injector helper + grantedOrigins storage repo + adapter registry entry + i18n keys + 单元测试 (ADO-03, ADO-06, ADO-07) ✓
- [x] 04-02-PLAN.md — OpenClaw adapter content script: match/compose/send/canDispatch + 单元测试 (ADO-01, ADO-02, ADO-04) ✓

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 04-03-PLAN.md — Integration: dispatch-pipeline permission guard + SendForm permission request + ErrorBanner extension + GrantedOriginsSection (ADO-06, ADO-07) ✓
...
**Wave 1**
- [x] 05-01-PLAN.md — Foundation: discord-format.ts (compose + truncation + escape) + registry entry + webNavigation permission + i18n keys + 单元测试 (ADD-01, ADD-02, ADD-07, ADD-08) ✓

**Wave 2** *(并行 — depends on 05-01)*
- [x] 05-02-PLAN.md — Discord adapter content script: ARIA selector + ClipboardEvent paste + rate limit + channel safety + DOM fixture + selector 单元测试 (ADD-01, ADD-03, ADD-04, ADD-05, ADD-06, ADD-07, ADD-09) ✓
- [x] 05-03-PLAN.md — Integration: background.ts webNavigation listener + dispatch-pipeline login detection + PlatformIcon Discord SVG + SendForm ToS footnote (ADD-04, ADD-06, ADD-07, ADD-08) ✓
...
- [x] 05-05-PLAN.md — Fix paste ISOLATED->MAIN world bridge + Discord icon SVG + login timeout race (ADD-01, ADD-03, ADD-06, ADD-08) ✓

**Wave 5** *(gap closure — UAT Test 1/4 residual failures)*
- [x] 05-06-PLAN.md — Fix editor clear after send (Escape after Enter) + increase ADAPTER_RESPONSE_TIMEOUT_MS 10s→20s (ADD-03, ADD-04) ✓
...
- [x] 06-1a-i18n-infra-PLAN.md — Signal-based i18n rewrite: shared/i18n/index.ts + localeItem + Vite YAML plugin（I18N-02）✓
- [x] 06-1b-i18n-tests-PLAN.md — Async locale init in popup/options（I18N-02）✓
- [x] 06-2-eslint-rule-PLAN.md — ESLint 硬编码字符串完整规则 + fixture 测试（I18N-03）✓
- [x] 06-3-manifest-verify-PLAN.md — Manifest __MSG_*__ 完整性验证（I18N-04）✓

**Wave 2** *(blocked on Wave 1 completion, specifically 06-1a)*
- [x] 06-4-language-section-PLAN.md — LanguageSection UI + locale 文件 key 更新（I18N-02）✓

**Wave 3** *(blocked on Wave 2 completion)*
- [x] 06-5-coverage-audit-PLAN.md — i18n coverage 审计脚本 + test:i18n-coverage（I18N-01）✓
...
**Plans**：4 plans（1 wave — 全并行，无文件冲突）

Plans:

**Wave 1** *(全并行 — 四个 plan 无文件重叠)*
- [x] 07-01-PLAN.md — Build verification tooling: verify-zip.ts + verify-readme-anchors.ts + wxt.config.ts zip.exclude + package.json scripts (DST-01, DST-03, DST-04) ✓ Complete
...
- [x] 07-04-PLAN.md — CWS store listing: STORE-LISTING.md (zh_CN) + STORE-LISTING.en.md (DST-01) ✓ Complete
...
| 6. i18n 加固 + 打磨    | 6/6            | Complete    | 2026-05-07 |
| 7. 分发上架            | 4/4            | Complete    | 2026-05-07 |
