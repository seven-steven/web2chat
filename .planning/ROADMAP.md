...
- [ ] **Phase 1: 扩展骨架 (Foundation)** - 基于 WXT 的 MV3 脚手架，从第一天起就接入类型化消息、存储 schema 与 i18n
...
1. WXT 解包构建可通过 `chrome://extensions → Load unpacked` 加载并显示工具栏 action 图标，manifest 仅声明 `activeTab` + `scripting` + `storage`（无 `<all_urls>`）；评审者可从 `dist/manifest.json` 中确认这一点。
...
5. `vitest` 与 `@playwright/test`（配合 `chromium.launchPersistentContext --load-extension`）跑绿；CI workflow 校验构建产物中没有任何 `<all_urls>` 引用，popup TSX 中没有任何硬编码的用户可见字符串。
...
**目标 (Goal)**：用户在任意页面上点击 action 图标，选择一个带 prompt 的 OpenClaw `http://localhost:18789/chat?session=agent:<a>:<s>` 目标，5s 内一条格式化消息落入正在运行的 OpenClaw 会话中，popup 反映成功状态 — 这是投递链路在友好目标上的首次端到端验证。
**依赖 (Depends on)**：Phase 3
**Requirements**：ADO-01, ADO-02, ADO-03, ADO-04, ADO-05, ADO-06
**成功标准**（必须为 TRUE 的条件）：

1. OpenClaw bundle 的 `host_permissions` 恰好包含 `http://localhost:18789/*`（由 manifest 校验测试断言）；不引入更广泛的权限。
2. `match()` 识别 OpenClaw URL 模式，并从 query 中解析出 `agent_name` + `session_name` — Vitest 单元测试覆盖合法、畸形和带尾随斜杠的变体。
...
5. `canDispatch` 探针区分 "OpenClaw 服务未运行"（连接被拒）与 "input 未找到"，并以经过 i18n 本地化的人性化文案 surfacing 结构化错误 `OPENCLAW_OFFLINE`（"OpenClaw 未在 localhost:18789 上运行 — 请启动后重试"）；Playwright 覆盖离线路径。
...
**目标 (Goal)**：评审者可以从一个打包好的 zip 安装 Web2Chat，阅读到与实际数据处理一致的隐私政策，看到收窄的 `host_permissions`（同时为 v2 平台保留 `optional_host_permissions`），并按双语 README 完成安装、使用、OpenClaw 设置以及 Discord ToS 注意事项的指引。
...
3. `manifest.json` 的 `host_permissions` 仅包含 `http://localhost:18789/*` 与 `https://discord.com/*`；`optional_host_permissions` 声明为空数组（或带文档化的占位项），以便 v2 适配器可以在安装/使用时申请 host，而不必让现有用户走一次权限升级提示。
...
| ---------------------- | -------------- | ----------- | --------- |
| 1. 扩展骨架            | 0/TBD          | Not started | -         |
| 2. 抓取流水线          | 0/TBD          | Not started | -         |
